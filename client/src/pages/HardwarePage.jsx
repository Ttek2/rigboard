...
import { HardDrive, Plus, Trash2, ChevronDown, ChevronRight, Wrench, Clock, AlertTriangle, DollarSign, Upload, Image, History, X } from 'lucide-react';
    ...
    <div className="ml-4 border-l" style={{ borderColor: 'var(--border)' }}>
      <div className="pl-4 py-2">
        <div className="flex items-center gap-2">
          ...
          {component.warranty_expires && new Date(component.warranty_expires) < new Date() && (
            <span className="text-xs text-amber-400">Warranty expired</span>
          )}
        </div>
...
export default function HardwarePage() {
  ...
  const [comp, setComp] = useState({ category: 'CPU', name: '', model: '', purchase_price: '', currency: 'EUR', warranty_expires: '', parent_component_id: '' });
  ...
  const handleAddComponent = async (e) => {
    ...
    setComp({ category: 'CPU', name: '', model: '', purchase_price: '', currency: 'EUR', warranty_expires: '', parent_component_id: '' });
            ...
            {showAddComponent && (
              <form onSubmit={handleAddComponent} className="mb-4 p-4 rounded-xl border space-y-3"
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <input value={comp.name} onChange={e => setComp({ ...comp, name: e.target.value })} placeholder="Name" required
                    className="px-3 py-2 rounded-lg border text-sm"
                    style={{ backgroundColor: 'var(--bg-primary)', borderColor: 'var(--border)', color: 'var(--text-primary)' }} />
                  <input value={comp.model} onChange={e => setComp({ ...comp, model: e.target.value })} placeholder="Model"
                    className="px-3 py-2 rounded-lg border text-sm"
                    style={{ backgroundColor: 'var(--bg-primary)', borderColor: 'var(--border)', color: 'var(--text-primary)' }} />
                  <input type="number" step="0.01" value={comp.purchase_price} onChange={e => setComp({ ...comp, purchase_price: e.target.value })} placeholder="Price"
                    className="px-3 py-2 rounded-lg border text-sm"
                    style={{ backgroundColor: 'var(--bg-primary)', borderColor: 'var(--border)', color: 'var(--text-primary)' }} />
                  <input type="date" value={comp.warranty_expires} onChange={e => setComp({ ...comp, warranty_expires: e.target.value })} placeholder="Warranty expires"
                    className="px-3 py-2 rounded-lg border text-sm"
                    style={{ backgroundColor: 'var(--bg-primary)', borderColor: 'var(--border)', color: 'var(--text-primary)' }} />
